...
class IndexView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if sort == 'best':
            student_statistics.sort(key=lambda x: float(x['average']) if x['average'] != '-' else -1, reverse=True)
        elif sort == 'worst':
            student_statistics.sort(key=lambda x: float(x['average']) if x['average'] != '-' else 999)
...
class StudentUpdateView(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        existing_grades = {g.subject_id: g for g in Grade.objects.filter(student=self.object)}
        ...
        for key, value in request.POST.items():
            if key.startswith('subject_'):
                ...
                if value:
                    ...
                    Grade.objects.filter(student=self.object, subject_id=subject_id).delete()
